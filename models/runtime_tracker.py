...
class RuntimeTracker:
    def __init__(self, det_score_thresh: float = 0.7, track_score_thresh: float = 0.6,
                 ...
                 history_len: int = 5): # [新增] history_len 参数
        ...
        self.history_len = history_len # [新增]
    ...
    def update(self, model_outputs: dict, tracks: List[TrackInstances]):
        ...
        # [创新点核心] 维护历史轨迹 Buffer
        # =========================================================================
        if hasattr(tracks[0], 'ref_pts_history'):
            # 这里的 boxes 是 cxcywh 格式，我们取前两个 (cx, cy)
            current_pts = tracks[0].boxes[..., :2]
            
            # 更新逻辑：丢弃最旧的一帧 (index 0)，在末尾 (index -1) 加上当前帧
            # tracks[0].ref_pts_history shape: [N, 5, 2]
            # cat([N, 4, 2], [N, 1, 2]) -> [N, 5, 2]
            tracks[0].ref_pts_history = torch.cat(
                [tracks[0].ref_pts_history[:, 1:, :], current_pts.unsqueeze(1)], 
                dim=1
            )
        else:
            # 如果是刚加载模型第一帧，可能还没有这个属性，初始化一下
            # 用当前帧填满整个 history
            current_pts = tracks[0].boxes[..., :2]
            tracks[0].ref_pts_history = current_pts.unsqueeze(1).repeat(1, self.history_len, 1)
        # =========================================================================
        ...
        # [新增] 初始化新目标的轨迹历史
        # 因为是第一帧出现，没有历史，所以用当前位置 copy 5次填满 buffer
        current_pts = new_tracks.boxes[..., :2] # [M, 2]
        new_tracks.ref_pts_history = current_pts.unsqueeze(1).repeat(1, self.history_len, 1) # [M, 5, 2]
